...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .navbar-logo { color: var(--white); font-family:'Syne',sans-serif; font-size:1.2rem; font-weight:800; letter-spacing:.5px; }
  ...
  .how-inner .section-title, .how-inner .section-sub { color:var(--white); }
  ...
  .step-arrow { display:flex; align-items:center; justify-content:center; color:#475569; padding-top:1rem; font-size:1.2rem; }
  ...
  .auth-logo { text-align:center; font-family:'Syne',sans-serif; font-weight:800; font-size:1.1rem; background:var(--navy); color:var(--white); border-radius:8px; padding:.5rem 1.5rem; display:inline-block; margin-bottom:1.25rem; }
  ...
  .google-btn { width:100%; padding:.65rem; border:1.5px solid var(--border); background:var(--white); border-radius:var(--radius-sm); font-family:inherit; font-size:.88rem; font-weight:600; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:.5rem; transition:background .2s; }
  ...
  /* ─── Dashboard layout ─── */
  .app-layout { display:flex; min-height:100vh; background:var(--offwhite); }
  .sidebar { width:220px; background:var(--navy); display:flex; flex-direction:column; padding:1.5rem 0; flex-shrink:0; }
  .sidebar-logo { font-family:'Syne',sans-serif; font-weight:800; color:var(--white); font-size:1rem; padding:0 1.25rem 1.5rem; border-bottom:1px solid #1e3a5f; margin-bottom:.75rem; }
  .sidebar-item { display:flex; align-items:center; gap:.65rem; padding:.65rem 1.25rem; cursor:pointer; color:#94a3b8; font-size:.875rem; font-weight:500; transition:all .15s; border-radius:0; }
  ...
  .topbar-greeting span { font-size:1.1rem; }
  .topbar-right { display:flex; align-items:center; gap:1rem; }
  .notif-btn { background:var(--offwhite); border:1px solid var(--border); border-radius:8px; padding:.35rem .75rem; font-size:.82rem; cursor:pointer; display:flex; align-items:center; gap:.3rem; }
  .avatar { width:32px; height:32px; background:var(--accent); border-radius:50%; display:flex; align-items:center; justify-content:center; color:var(--white); font-size:.8rem; font-weight:700; cursor:pointer; }
  ...
  .card-title { font-weight:700; font-size:.88rem; margin-bottom:.85rem; color:var(--text); }
  ...
  .factor-bar { height:100%; border-radius:4px; transition:width .6s; }
  ...
  .rec-blue { background:#dbeafe; color:var(--accent); }
  .rec-teal { background:#e0f2fe; color:var(--teal); }
  .rec-green { background:#dcfce7; color:#16a34a; }
  .rec-orange { background:#ffedd5; color:var(--orange); }
  ...
  .trend-chart { background:var(--offwhite); border-radius:8px; padding:1rem; position:relative; height:120px; overflow:hidden; }
  .trend-line { position:absolute; bottom:20px; left:10px; right:10px; }

  /* ─── Quick actions ─── */
  .qa-btn { width:100%; display:flex; align-items:center; justify-content:center; gap:.5rem; padding:.65rem; border-radius:var(--radius-sm); font-size:.85rem; font-weight:600; cursor:pointer; border:none; transition:all .2s; margin-bottom:.6rem; }
  .qa-primary { background:var(--accent); color:var(--white); }
  .qa-primary:hover { background:var(--accent2); }
  .qa-secondary { background:var(--offwhite); border:1.5px solid var(--border); color:var(--text); }
  .qa-secondary:hover { background:var(--border); }

  /* ─── Assessment Form ─── */
  .progress-bar-outer { display:flex; align-items:center; gap:0; margin-bottom:2rem; }
  .progress-step { display:flex; flex-direction:column; align-items:center; flex:1; }
  .progress-circle { width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:.85rem; font-weight:700; border:2px solid var(--border); background:var(--white); color:var(--muted); transition:all .3s; }
  ...
  .progress-label { font-size:.72rem; color:var(--muted); margin-top:.35rem; font-weight:500; }
  ...
  .slider-label { width:140px; font-size:.83rem; color:var(--text2); flex-shrink:0; }
  ...
  .slider-val { width:60px; text-align:right; font-size:.83rem; font-weight:700; color:var(--text); }
  ...
  .bar-chart-bar { width:100%; border-radius:4px 4px 0 0; transition:height .5s; }
  .bar-chart-label { font-size:.7rem; color:var(--text2); text-align:center; }
  .reported-row { display:flex; align-items:center; gap:1rem; margin-bottom:.6rem; }
  .rep-label { width:100px; font-size:.78rem; color:var(--text2); }
  .rep-bar { flex:1; }
  .rep-bar-inner { height:6px; border-radius:3px; }
  .rep-reported { background:#cbd5e1; }
  .rep-actual { background:var(--accent); }
  .rep-val { font-size:.78rem; font-weight:600; width:30px; text-align:right; }
  .rep-tag { font-size:.68rem; color:var(--muted); }
  ...
  .history-table thead tr { background:var(--offwhite); }
  .history-table th { text-align:left; padding:.65rem 1rem; font-size:.78rem; font-weight:700; color:var(--text2); border-bottom:1px solid var(--border); }
  ...
  .view-btn:hover { background:var(--blue); }
...
const Navbar = ({ onNav }) => (
  <nav className="navbar">
    ...
    <div className="navbar-links">
      ...
      <button className="btn btn-primary" onClick={() => onNav("assessment")}>Start Assessment</button>
    </div>
  </nav>
);
      ...
      <div className="hero-inner">
        <div>
          ...
          <div className="hero-actions">
            <button className="btn btn-primary btn-lg" onClick={() => onNav("assessment")}>Take Assessment</button>
    ...
    <section className="features-section">
      ...
      <div className="features-grid">
        {[
          ...
          { icon:"📊", title:"Stress Analytics", desc:"Track stress patterns and key contributing factors" },
    ...
    <section className="how-section">
      <div className="how-inner">
        ...
        <div className="steps-grid">
          {[
            { icon:"📝", title:"Fill Assessment", desc:"Answer a few questions about your routine and feelings" },
...
const LoginPage = ({ onNav }) => (
  <div className="auth-page">
    <div className="auth-card">
      ...
      <button className="btn btn-primary btn-block" onClick={() => onNav("dashboard")}>Login</button>
...
// ─── SIDEBAR + LAYOUT ─────────────────────────────────────────────────────────
const AppLayout = ({ active, onNav, children }) => {
  const items = [
  ...
  return (
    <div className="app-layout">
      <aside className="sidebar">
        <div className="sidebar-logo">🧠 BurnoutAI</div>
        {items.map(it => (
          <div key={it.id} className={`sidebar-item ${active===it.id?"active":""}`} onClick={() => onNav(it.id)}>
...
const SimpleTrendChart = () => {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="mini-chart">
      ...
      <path d={d} fill="none" stroke="#2563eb" strokeWidth="2.5" strokeLinecap="round" />
      {pts.map((v, i) => <circle key={i} cx={toX(i)} cy={toY(v)} r="3.5" fill="#2563eb" />)}
    </svg>
  );
};

const DashboardPage = ({ onNav }) => (
  <AppLayout active="dashboard" onNav={onNav}>
    <div className="topbar">
      <div className="topbar-greeting">Welcome back, Student! <span>👋</span></div>
    ...
    <div className="page-content">
      <div className="grid-3">
        ...
        <div className="card">
          ...
          {[
            { label:"Sleep", pct:85, cls:"bar-red", level:"High" },
            { label:"Screen Time", pct:80, cls:"bar-orange", level:"High" },
            { label:"Study Hours", pct:55, cls:"bar-yellow", level:"Medium" },
            { label:"Exam Pressure", pct:50, cls:"bar-yellow", level:"Medium" },
          ].map((f, i) => (
            <div className="factor-row" key={i}>
              ...
              <span className="bar-level" style={{ color: f.cls === "bar-red" ? "#ef4444" : f.cls === "bar-orange" ? "#f97316" : "#eab308" }}>{f.level}</span>
            </div>
          ))}
        </div>
...
// ─── ASSESSMENT PAGE ──────────────────────────────────────────────────────────
const SliderRow = ({ label, value, onChange, unit = "/ 10" }) => (
...
const AssessmentPage = ({ onNav }) => {
  ...
  return (
    <AppLayout active="assessment" onNav={onNav}>
      <div className="topbar">
        <div className="topbar-greeting">Assessment Form</div>
      </div>
      <div className="page-content">
        <div className="card" style={{ maxWidth:700, margin:"0 auto" }}>
          ...
          <div className="progress-bar-outer">
            {steps.map((s, i) => (
              <>
                ...
                {i < steps.length - 1 && <div className={`progress-connector ${i+1 < step ? "done" : ""}`} key={`c${i}`} />}
              </>
            ))}
          </div>
          ...
          {sections[step-1].fields.map(f => (
            <SliderRow key={f.key} label={f.label} value={vals[f.key]} onChange={set(f.key)} unit={f.unit || "/ 10"} />
          ))}

          <div style={{ display:"flex", justifyContent:"space-between", marginTop:"1.5rem" }}>
            <button className="btn btn-secondary" style={{ padding:".55rem 1.25rem" }}
              onClick={() => step > 1 && setStep(p => p-1)} disabled={step===1}>
              ← Back
            </button>
            {step < 4
...
const ResultsPage = ({ onNav }) => (
  <AppLayout active="results" onNav={onNav}>
        ...
        <div className="card">
          ...
          <div style={{ background:"#fee2e2", borderRadius:10, padding:"1rem", border:"1px solid #fecaca" }}>
            <div style={{ display:"flex", alignItems:"center", gap:".5rem", marginBottom:".5rem" }}>
              ...
              <strong style={{ color:"#ef4444", fontSize:".95rem" }}>HIGH RISK</strong>
            </div>
        ...
        <div className="card">
          ...
          {[
            ...
            <div key={r.label} style={{ marginBottom:".75rem" }}>
              <div style={{ fontSize:".78rem", fontWeight:700, marginBottom:".3rem" }}>{r.label}</div>
...
const HistoryPage = ({ onNav }) => {
  ...
  return (
    <AppLayout active="history" onNav={onNav}>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i}>
                  ...
                  <td>
                    <span className={`badge ${r.risk==="High"?"badge-high":r.risk==="Moderate"?"badge-moderate":"badge-low"}`}>{r.risk}</span>
                  </td>
...
const SettingsPage = ({ onNav }) => (
  <AppLayout active="settings" onNav={onNav}>
...
function SimpleTrendChart() {
  ...
  const d = pts.map((v, i) => `${i === 0 ? "M" : "L"} ${toX(i)} ${toY(v)}`).join(" ");
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="mini-chart" style={{ overflow:"visible" }}>
      <defs>
      ...
      <path d={d + ` L ${w} ${h} L 0 ${h} Z`} fill="url(#grad2)" />
...
export default function App() {
  ...
  const pages = {
    home: <LandingPage onNav={setPage} />,
    login: <LoginPage onNav={setPage} />,
    dashboard: <DashboardPage onNav={setPage} />,
    assessment: <AssessmentPage onNav={setPage} />,
    results: <ResultsPage onNav={setPage} />,
    history: <HistoryPage onNav={setPage} />,
    settings: <SettingsPage onNav={setPage} />,
  };
